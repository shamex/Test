 ...
 */
@Entity
@Table(name = "diseasesymptoms", catalog = "feobet", schema = "")
@XmlRootElement
@NamedQueries({
    @NamedQuery(name = "Diseasesymptoms.findAll", query = "SELECT d FROM Diseasesymptoms d"),
    @NamedQuery(name = "Diseasesymptoms.findById", query = "SELECT d FROM Diseasesymptoms d WHERE d.id = :id")})
public class Diseasesymptoms implements Serializable {
    ...
    private Integer id;
    @JoinColumn(name = "symptomsid", referencedColumnName = "id")
    @ManyToOne
    ...
    public String toString() {
        return "com.um.feobet.persistence.entity.Diseasesymptoms[ id=" + id + " ]";
    }
